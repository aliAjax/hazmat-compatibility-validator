...
func (e Engine) Evaluate(revision manifest.Revision, book rulebook.Rulebook, resolved map[string]substance.Substance) Evaluation {
	...
	for _, c := range revision.Containers {
		containers[c.ID] = c
	}
	...
	for left := 0; left < len(revision.Items); left++ {
		for right := left + 1; right < len(revision.Items); right++ {
			a, b := revision.Items[left], revision.Items[right]
